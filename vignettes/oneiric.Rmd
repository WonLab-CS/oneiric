...
export_simulation(spatial = chaos_map,
    cells = chaos_counts,
    out_dir = output,
...
circles <- do.call("rbind", circular_layer)
circles$Territory <- as.factor(circles$Territory)
g <- ggplot(circles, aes(x = x, y = y, col = Territory)) +
    geom_point(size = 0.5) +
    theme_bw() +
...
export_simulation(spatial = circular_layer,
    cells = circular_layer_counts,
    out_dir = output,
...
chaos <- do.call("rbind", chaos_layer)
chaos$Territory <- as.factor(chaos$Territory)
g <- ggplot(chaos, aes(x = x, y = y, col = Territory)) +
    geom_point(size = 0.5) +
    theme_bw() +
...
export_simulation(spatial = chaos_layer,
    cells = chaos_layer_counts,
    out_dir = output,
    file_tag = "layered_tinker_spatial_territories")
